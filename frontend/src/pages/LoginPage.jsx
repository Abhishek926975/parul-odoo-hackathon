    ...
    <div className="mx-auto grid min-h-[calc(100vh-73px)] max-w-7xl items-center gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[1.1fr_.9fr] lg:px-8">
      ...
      <form className="traveloop-card mx-auto w-full max-w-md p-8" onSubmit={handleSubmit}>
        ...
        <div className="mt-4 flex items-center justify-between text-sm text-traveloop-muted">
          ...
          <button className="font-medium text-traveloop-sand-dark" type="button" onClick={() => setForgotOpen(true)}>
            Forgot password?
          </button>
        </div>
      ...
      <Modal open={forgotOpen} title="Reset password" onClose={() => setForgotOpen(false)}>
        <form
          ...
          onSubmit={(event) => {
            event.preventDefault();
            showToast("info", `Password reset link sent to ${forgotEmail || "your email"}`);
            setForgotOpen(false);
            setForgotEmail("");
          }}
        >
          <input className="traveloop-input" required type="email" placeholder="you@example.com" value={forgotEmail} onChange={(event) => setForgotEmail(event.target.value)} />
