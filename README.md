...
If you already have PostgreSQL installed locally, set `backend/.env` to your own credentials. For example, this local install was verified with:

```env
DATABASE_URL=postgresql://postgres:admin123@localhost:5432/traveloop
```

If you prefer the Docker database, run:

```bash
docker compose up -d db
```

Then keep the example URL in `backend/.env`, which uses host port `5433`.
